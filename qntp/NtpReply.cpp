...
//-------------------------------------------------------------------------
QDateTime NtpReply::ServerTime() const
{
QDateTime tc1,tc2,ts1,ts2;

    tc1 =NtpTimestamp::toDateTime(d->packet.basic.originateTimestamp);
    tc2 =d->destinationTime;
    ts1 =NtpTimestamp::toDateTime(d->packet.basic.receiveTimestamp);
    ts2 =NtpTimestamp::toDateTime(d->packet.basic.transmitTimestamp);


return (ts2.addMSecs((tc1.msecsTo(tc2) - ts1.msecsTo(ts2)) / 2));
}
